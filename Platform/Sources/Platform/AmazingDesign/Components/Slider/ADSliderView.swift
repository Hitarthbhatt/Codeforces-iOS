...
//  Platform
//
//  Created by Ankit Panchotiya on 19/10/25.
//

import SwiftUI

public struct ADSliderView<S1: ShapeStyle, S2: ShapeStyle, T: View>: View {
...
public extension ADSliderView where T == Never, S1 == Color, S2 == Color {
    init<V>(
        value: Binding<V>,
...
public extension ADSliderView  {
    init<V>(
        value: Binding<V>,
        in bounds: ClosedRange<V>,
        step: V.Stride? = nil,
        barStyle: (height: Double?, cornerRadius: Double),
        fillBackground: S1,
        fillTrack: S2,
...
struct ThumbView<S: ShapeStyle, T: View>: View {
